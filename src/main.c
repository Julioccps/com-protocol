...
protocol_frame_t mount_frame(const uint8_t* message, uint8_t id, uint8_t msg_length){
    ...
    frame.checksum = calculate_check_sum(checksum_data, 2 + msg_length);
...
int verify_checksum(const protocol_frame_t* frame){
    ...
    uint8_t calculated_checksum = calculate_check_sum(checksum_data, 2 + frame->length);
